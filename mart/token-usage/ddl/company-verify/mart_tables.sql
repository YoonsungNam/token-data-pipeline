...
-- source: mart/token-usage/ddl/company/mart_tables.sql
...
-- -------------------------------------------------------------
-- 1) token_verify_mart.token_usage_1d — 사용자×모델 일별 상세 + 조직/비용 부착
--    Grain: date × service × user_id × user_type × model
--    token_verify_fact.raw_token_usage_1d와 co-location (동일 파티션/ORDER BY/샤딩키)
-- -------------------------------------------------------------

CREATE TABLE IF NOT EXISTS token_verify_mart.token_usage_1d_local
...
ENGINE = ReplicatedMergeTree(
    '/clickhouse/tables/{shard}/token_verify_mart/token_usage_1d_local',
    '{replica}'
)
...
CREATE TABLE IF NOT EXISTS token_verify_mart.token_usage_1d_dist
ON CLUSTER 'gpu-monitoring'
(
    date                  Date,
...
ENGINE = Distributed('gpu-monitoring', 'token_verify_mart', 'token_usage_1d_local',
                     cityHash64(service, user_id));

-- -------------------------------------------------------------
-- 2) token_verify_mart.agg_token_service_1d — 서비스 일별 집계 + 보고값 대사
--    Grain: date × service_group × service
--    reported_*는 token_verify_fact.raw_token_usage_summary_1d 조인값 —
--    diff_* = Σdetail − reported, is_derived=1이면 NULL (§4.1 파생 시맨틱)
-- -------------------------------------------------------------

CREATE TABLE IF NOT EXISTS token_verify_mart.agg_token_service_1d_local
ON CLUSTER 'gpu-monitoring'
(
    date                               Date,
    service_group                      LowCardinality(String),
...
ENGINE = ReplicatedMergeTree(
    '/clickhouse/tables/{shard}/token_verify_mart/agg_token_service_1d_local',
    '{replica}'
)
...
CREATE TABLE IF NOT EXISTS token_verify_mart.agg_token_service_1d_dist
ON CLUSTER 'gpu-monitoring'
(
    date                               Date,
    service_group                      LowCardinality(String),
...
ENGINE = Distributed('gpu-monitoring', 'token_verify_mart', 'agg_token_service_1d_local',
                     cityHash64(service));

-- -------------------------------------------------------------
-- 3) token_verify_mart.agg_token_org_1d — 조직 일별 집계 (말단 경로 단위)
--    Grain: date × org_path — 상위 롤업은 쿼리 시 arraySlice GROUP BY,
--    서브트리 질의 표준 = prefix 비교 (§4.3)
-- -------------------------------------------------------------

CREATE TABLE IF NOT EXISTS token_verify_mart.agg_token_org_1d_local
ON CLUSTER 'gpu-monitoring'
(
    date                  Date,
...
ENGINE = ReplicatedMergeTree(
    '/clickhouse/tables/{shard}/token_verify_mart/agg_token_org_1d_local',
    '{replica}'
)
PARTITION BY toYYYYMM(date)
...
CREATE TABLE IF NOT EXISTS token_verify_mart.agg_token_org_1d_dist
ON CLUSTER 'gpu-monitoring'
(
    date                  Date,
    org_path              Array(String),
...
ENGINE = Distributed('gpu-monitoring', 'token_verify_mart', 'agg_token_org_1d_local',
                     cityHash64(arrayStringConcat(org_path, '>')));

-- -------------------------------------------------------------
-- 4) token_verify_mart.agg_token_model_1d — 모델 일별 집계
--    Grain: date × model × provider (§4.3)
-- -------------------------------------------------------------

CREATE TABLE IF NOT EXISTS token_verify_mart.agg_token_model_1d_local
ON CLUSTER 'gpu-monitoring'
(
    date                  Date,
...
ENGINE = ReplicatedMergeTree(
    '/clickhouse/tables/{shard}/token_verify_mart/agg_token_model_1d_local',
    '{replica}'
)
PARTITION BY toYYYYMM(date)
...
CREATE TABLE IF NOT EXISTS token_verify_mart.agg_token_model_1d_dist
ON CLUSTER 'gpu-monitoring'
(
    date                  Date,
    model                 LowCardinality(String),
...
ENGINE = Distributed('gpu-monitoring', 'token_verify_mart', 'agg_token_model_1d_local',
                     cityHash64(model));
