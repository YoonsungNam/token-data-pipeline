...
-- source: mart/token-metrics/ddl/company/mart_metrics_tables.sql
...
-- -------------------------------------------------------------
-- 1) token_verify_mart.agg_token_model_cost_1d — M1 모델 비용 C (grain: date × service × model(canon))
...
CREATE TABLE IF NOT EXISTS token_verify_mart.agg_token_model_cost_1d_local
...
(
    ...
    input_tokens          UInt64                  COMMENT 'token_verify_mart.token_usage_1d 합 (usage_svc 전 서비스)',
    ...
    model_registered      UInt8                   COMMENT 'canonical이 token_verify_dim.dim_token_model_alias에 identity/alias 행으로 존재(canon 히트)하면 1, 미등록 0 (설계 §2·§6.1)',
...
ENGINE = ReplicatedMergeTree(
    '/clickhouse/tables/{shard}/token_verify_mart/agg_token_model_cost_1d_local',
    '{replica}'
)
...
CREATE TABLE IF NOT EXISTS token_verify_mart.agg_token_model_cost_1d_dist
ON CLUSTER 'gpu-monitoring'
(
    date                  Date,
...
ENGINE = Distributed('gpu-monitoring', 'token_verify_mart', 'agg_token_model_cost_1d_local',
                     cityHash64(service));

-- -------------------------------------------------------------
-- 2) token_verify_mart.token_metrics_check_1d — M3 데이터 품질 검사 (9/14 알림 표면)
--    grain: date × service × check_name × model × gpu_type (model/gpu_type은 해당 없으면 '')
--    detail은 수·이름만 (데이터 원문·사용자 식별자 금지 — §5.6)
-- -------------------------------------------------------------

CREATE TABLE IF NOT EXISTS token_verify_mart.token_metrics_check_1d_local
ON CLUSTER 'gpu-monitoring'
(
    date          Date                              COMMENT 'KST 집계일',
    service_group LowCardinality(String),
...
ENGINE = ReplicatedMergeTree(
    '/clickhouse/tables/{shard}/token_verify_mart/token_metrics_check_1d_local',
    '{replica}'
)
PARTITION BY toYYYYMM(date)
...
CREATE TABLE IF NOT EXISTS token_verify_mart.token_metrics_check_1d_dist
ON CLUSTER 'gpu-monitoring'
(
    date          Date,
    service_group LowCardinality(String),
...
ENGINE = Distributed('gpu-monitoring', 'token_verify_mart', 'token_metrics_check_1d_local',
                     cityHash64(service));

-- -------------------------------------------------------------
-- 3) token_verify_mart.agg_token_model_share_1d — M4 공유 모델 배분 (P0-stretch; 정의서 3.6/3.9)
--    grain: date × model(canon) × service × provider_service
--    share = W(s)/W(m); allocated_cost_krw = model_cost × share (내부) / 벤더 단가 (external_api)
-- -------------------------------------------------------------

CREATE TABLE IF NOT EXISTS token_verify_mart.agg_token_model_share_1d_local
ON CLUSTER 'gpu-monitoring'
(
    date                Date                    COMMENT 'KST 집계일',
...
ENGINE = ReplicatedMergeTree(
    '/clickhouse/tables/{shard}/token_verify_mart/agg_token_model_share_1d_local',
    '{replica}'
)
PARTITION BY toYYYYMM(date)
...
CREATE TABLE IF NOT EXISTS token_verify_mart.agg_token_model_share_1d_dist
ON CLUSTER 'gpu-monitoring'
(
    date                Date,
    model               LowCardinality(String),
...
ENGINE = Distributed('gpu-monitoring', 'token_verify_mart', 'agg_token_model_share_1d_local',
                     cityHash64(model));

-- -------------------------------------------------------------
-- 4) token_verify_mart.agg_token_gpu_group_1d — M2 그룹 귀속·유휴 (P0-stretch; 정의서 3.1/3.3/3.4)
--    grain: date × service_group × gpu_type (쿼터 보유 단위)
--    I1: idle ≥ 0 (음수면 over_report=1 + 0 클램프) / I2: group_total = ΣC + test + idle + unattributed ± gap
-- -------------------------------------------------------------

CREATE TABLE IF NOT EXISTS token_verify_mart.agg_token_gpu_group_1d_local
ON CLUSTER 'gpu-monitoring'
(
    date                     Date                    COMMENT 'KST 집계일',
    service_group            LowCardinality(String),
...
ENGINE = ReplicatedMergeTree(
    '/clickhouse/tables/{shard}/token_verify_mart/agg_token_gpu_group_1d_local',
    '{replica}'
)
PARTITION BY toYYYYMM(date)
...
CREATE TABLE IF NOT EXISTS token_verify_mart.agg_token_gpu_group_1d_dist
ON CLUSTER 'gpu-monitoring'
(
    date                     Date,
    service_group            LowCardinality(String),
...
ENGINE = Distributed('gpu-monitoring', 'token_verify_mart', 'agg_token_gpu_group_1d_local',
