...
-- source: collectors/token-metrics/ddl/company/raw_token_metrics.sql
-- params: --fact=token_verify_fact --dim=token_verify_dim --mart=token_verify_mart --account=token_verify
-- =============================================================

-- =============================================================
-- Company/Stage ClickHouse DDL — token_verify_fact.raw_token_metrics_* + token_verify_fact.collect_audit_metrics_1d
...
-- -------------------------------------------------------------
-- 1) token_verify_fact.raw_token_metrics_gpu_1d — GPU 점유 (grain: date × service × model × gpu_type × category)
-- -------------------------------------------------------------

CREATE TABLE IF NOT EXISTS token_verify_fact.raw_token_metrics_gpu_1d_local
...
ENGINE = ReplicatedMergeTree(
    '/clickhouse/tables/{shard}/token_verify_fact/raw_token_metrics_gpu_1d_local',
    '{replica}'
)
...
CREATE TABLE IF NOT EXISTS token_verify_fact.raw_token_metrics_gpu_1d_dist
ON CLUSTER 'gpu-monitoring'
(
    date          Date,
...
ENGINE = Distributed('gpu-monitoring', 'token_verify_fact', 'raw_token_metrics_gpu_1d_local',
                     cityHash64(service));

-- -------------------------------------------------------------
-- 2) token_verify_fact.raw_token_metrics_serving_1d — 서빙 성능 long form
--    (grain: date × service × model × metric × name; 유일성은 정규화기가 (model)·(model, custom.name) 중복 제거 후 성립)
-- -------------------------------------------------------------

CREATE TABLE IF NOT EXISTS token_verify_fact.raw_token_metrics_serving_1d_local
ON CLUSTER 'gpu-monitoring'
(
    date          Date                   COMMENT 'KST 집계일',
    service_group LowCardinality(String) COMMENT '정본 = collectors/token-metrics/endpoints.yaml',
...
ENGINE = ReplicatedMergeTree(
    '/clickhouse/tables/{shard}/token_verify_fact/raw_token_metrics_serving_1d_local',
    '{replica}'
)
PARTITION BY toYYYYMM(date)
...
CREATE TABLE IF NOT EXISTS token_verify_fact.raw_token_metrics_serving_1d_dist
ON CLUSTER 'gpu-monitoring'
(
    date          Date,
    service_group LowCardinality(String),
...
ENGINE = Distributed('gpu-monitoring', 'token_verify_fact', 'raw_token_metrics_serving_1d_local',
                     cityHash64(service));

-- -------------------------------------------------------------
-- 3) token_verify_fact.raw_token_metrics_summary_1d — 응답당 정확히 1행(앵커), NODATA(rows==0)도 기록
--    (grain: date × service). 앵커 존재 = 적재 완료 — 정기 실행의 스킵 판정·M0 커버리지 근거.
-- -------------------------------------------------------------

CREATE TABLE IF NOT EXISTS token_verify_fact.raw_token_metrics_summary_1d_local
ON CLUSTER 'gpu-monitoring'
(
    date                   Date                              COMMENT 'KST 집계일',
    service_group          LowCardinality(String)            COMMENT '정본 = collectors/token-metrics/endpoints.yaml',
...
ENGINE = ReplicatedMergeTree(
    '/clickhouse/tables/{shard}/token_verify_fact/raw_token_metrics_summary_1d_local',
    '{replica}'
)
PARTITION BY toYYYYMM(date)
...
CREATE TABLE IF NOT EXISTS token_verify_fact.raw_token_metrics_summary_1d_dist
ON CLUSTER 'gpu-monitoring'
(
    date                   Date,
    service_group          LowCardinality(String),
...
ENGINE = Distributed('gpu-monitoring', 'token_verify_fact', 'raw_token_metrics_summary_1d_local',
                     cityHash64(service));

-- -------------------------------------------------------------
-- 4) token_verify_fact.collect_audit_metrics_1d — 교체 감사 (append-only: 절대 DELETE 안 함, GRANT도 INSERT만)
--    --replace 재수집이 기존 세대를 지우기 직전 요약을 보존 (마스터 §8.4 상속)
-- -------------------------------------------------------------

CREATE TABLE IF NOT EXISTS token_verify_fact.collect_audit_metrics_1d_local
ON CLUSTER 'gpu-monitoring'
(
...
ENGINE = ReplicatedMergeTree(
    '/clickhouse/tables/{shard}/token_verify_fact/collect_audit_metrics_1d_local',
    '{replica}'
)
PARTITION BY toYYYYMM(date)
...
CREATE TABLE IF NOT EXISTS token_verify_fact.collect_audit_metrics_1d_dist
ON CLUSTER 'gpu-monitoring'
(
    date               Date,
    service            LowCardinality(String),
...
ENGINE = Distributed('gpu-monitoring', 'token_verify_fact', 'collect_audit_metrics_1d_local',
                     cityHash64(service));
