...
def test_cli_accepts_utf8_bom_csv(tmp_path):
    ...
    bom_csv.write_bytes(b"\xef\xbb\xbf" + FIXTURES["gpu_tco"].read_bytes())
    out_path = tmp_path / "out.sql"
    result = _run("gpu_tco", "--effective-from", EF, csv_path=bom_csv, out_path=out_path)
    assert result.returncode == 0, result.stderr
    assert "출력 행수: 3 (NULL 숫자 셀 1)" in result.stdout


def test_cli_rejects_non_utf8_csv(tmp_path):
    marker = "가나다"
    text = FIXTURES["gpu_tco"].read_text(encoding="utf-8")
    assert marker not in text
    text = text.replace("—", "-")  # cp949로 인코딩 불가한 em dash 제거
    text = text.replace("합성 - 실제 TCO 아님", f"합성 - 실제 TCO 아님 {marker}")
    ...
    assert "H100" not in result.stderr
    assert not out_path.exists()


def test_cli_unwritable_out_reports_cleanly(tmp_path):
    out_path = tmp_path / "no_such_dir" / "x.sql"
    result = _run("gpu_tco", "--effective-from", EF, out_path=out_path)
    assert result.returncode == 1
