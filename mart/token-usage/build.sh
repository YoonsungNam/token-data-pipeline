#!/usr/bin/env bash
# token-mart 이미지 빌드/푸시 (스펙 §7.2 스크립트 규약)
#
# 사용법:
#   ./mart/token-usage/build.sh [--registry <registry>] [--tag <tag>] <stage|company>
#
...
echo "다음 단계:"
echo "  ./mart/token-usage/install.sh --registry ${REGISTRY} --tag ${TAG} ${ENV}"
