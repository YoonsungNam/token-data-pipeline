...
-- source: assets/model-catalog/ddl/company/seed_dim_token_model_alias.sql
-- params: --fact=token_verify_fact --dim=token_verify_dim --mart=token_verify_mart --account=token_verify
-- =============================================================

-- =============================================================
-- token_verify_dim.dim_token_model_alias 시드 (설계 2026-08-31 §4.2 — dim_holiday 3요소 패턴)
...
--   service_not_in_registry — 마지막은 token_verify_dim.dim_token_metrics_service_dist 대조. 레지스트리 테이블은
--   collectors/token-metrics/ddl/company/dim_token_metrics_service.sql(README 적용 순서 2단계)이 만들고 이 시드는
--   4단계에 적용되므로 항상 먼저 존재한다.
-- =============================================================

INSERT INTO token_verify_dim.dim_token_model_alias_dist
...
WHERE (alias, effective_from) NOT IN (
    SELECT alias, effective_from FROM token_verify_dim.dim_token_model_alias_dist
)
...
FROM token_verify_dim.dim_token_model_alias_dist
...
WHERE alias != canonical
  AND canonical GLOBAL IN (
      SELECT alias FROM token_verify_dim.dim_token_model_alias_dist WHERE alias != canonical
  )
...
WHERE defining_service != ''
  AND defining_service GLOBAL NOT IN (
      SELECT service FROM token_verify_dim.dim_token_metrics_service_dist
  );
