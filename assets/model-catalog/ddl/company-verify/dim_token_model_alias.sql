...
-- source: assets/model-catalog/ddl/company/dim_token_model_alias.sql
-- params: --fact=token_verify_fact --dim=token_verify_dim --mart=token_verify_mart --account=token_verify
-- =============================================================

-- =============================================================
-- Company/Stage ClickHouse DDL — token_verify_dim.dim_token_model_alias (설계 2026-08-31 §4.2, P0)
...
-- model_registered 판정: canonical이 dim_token_model_alias에 identity 행(생성기 자동 생성)으로 존재하면 1 — token_verify_dim.dim_token_model 조회 없음(6c 읽기 계약 3테이블 고정).
-- =============================================================

CREATE TABLE IF NOT EXISTS token_verify_dim.dim_token_model_alias_local
...
ENGINE = ReplicatedMergeTree(
    '/clickhouse/tables/{shard}/token_verify_dim/dim_token_model_alias_local',
    '{replica}'
)
ORDER BY (alias, effective_from)
SETTINGS index_granularity = 8192;

CREATE TABLE IF NOT EXISTS token_verify_dim.dim_token_model_alias_dist
ON CLUSTER 'gpu-monitoring'
(
    alias            String,
    effective_from   Date,
...
ENGINE = Distributed('gpu-monitoring', 'token_verify_dim', 'dim_token_model_alias_local', cityHash64(alias));
